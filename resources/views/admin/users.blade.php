            ...
            @foreach($users as $user)
                @if(!$user->isAdmin)
                    <tr class="bg-white rounded-md text-center border hover:bg-gray-50">
                        <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap text-center">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="mb-4">
                                <a href="{{ route('admin.editUser',  ['id' => $user->id]) }}" class="inline-block rounded bg-indigo-600 px-8 py-3 text-sm font-medium text-white transition hover:scale-110 hover:shadow-xl focus:outline-none focus:ring active:bg-indigo-500">
                                    Consulter le profil
                                </a>
                            </div>
                            <div class="">
                                <form action="{{ route("admin.destroyUser", ["id"=>$user->id]) }}" method="post">
                                    @method("DELETE")
                                    <button class="inline-block rounded bg-red-600 px-8 py-3 text-sm font-medium text-white transition hover:scale-110 hover:shadow-xl focus:outline-none focus:ring active:bg-red-500">
