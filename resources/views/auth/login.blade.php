...
@section('content')
    <div class="w-2/6 border shadow-xl p-10 rounded-2xl border-gray-200 bg-gray-100">
        <a href="{{ route('home') }}" class=" w-16 inline-block rounded-2xl border border-blue-600 bg-blue-600 p-3 text-white hover:bg-transparent hover:text-indigo-600 focus:outline-none focus:ring active:text-indigo-500">
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <button class="mt-4 w-full inline-block rounded-2xl rounded bg-indigo-600 px-8 py-3 text-sm font-medium text-white transition hover:scale-110 hover:shadow-xl focus:outline-none focus:ring active:bg-indigo-500">
                Se Connecter
            </button>
        </form>
    </div>
